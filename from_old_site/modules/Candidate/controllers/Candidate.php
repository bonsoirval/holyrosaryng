	...
	*/
	...
	public function profile()
	{
            ...
            if($this->input->post()){
                ...
                if($this->form_validation->run() === FALSE)
                {
                    ...
                    if($this->candidate->profile() == TRUE)
                    {
                        ...
                        $this->data ['passport'] = $this->candidate->get_passport();
                        redirect('Candidate/passport_upload','refresh');
        ...
        public function passport_upload()
        {
            ...
            if ($this->input->post('passport_upload')) {
                ...
                if (!$is_file_error) {
                    ...
                    if (!$this->upload->do_upload('passport')) {
                        ...
                        $this->data ['passport'] = $this->candidate->get_passport();
                        $this->template->load('Candidate', 'default', 'profile', $this->data);
                        redirect('Candidate/contact', 'refresh');
                    }
                }
            ...
            $this->data['title'] = "Passport Upload";
            $this->template->load('Candidate', 'default', 'passport_upload', $this->data);
        }
        ...
        public function contact()
        {
            ...
            if($this->form_validation->run() == false)
            {
                    ...
                    if($this->candidate->update_contact() == true)
                    {
                            ...
                            redirect("Candidate/next_kin", 'refresh'); 
                            //$this->template->load('Candidate', 'default', 'contact', $this->data);
                    }
                    else{
    ...
    if($this->input->post())
    {
            ...
            if($this->form_validation->run() == false)
            {
                    ...
                    if($this->candidate->update_next_kin() == true)
                    {
                            ...
                            $this->data['title'] = 'Candidate Contact';
                            redirect("Candidate/olevel", 'refresh');
                            $this->template->load('Candidate', 'default', 'contact', $this->data);
                    }
                    else{
                        echo "Contact update failed";
            ...
            $this->data['title'] = 'Next of Kin Contact';
            $this->template->load("Candidate", 'default', 'nextkincontact', $this->data);
            //echo "contact";
        }
}
        ...
        {//when there is input from user
            ...
            {//check validation failure first for security
                ...
                {//function return false === update failed
                    ...
                    $this->data['title'] = 'Candidate Olevel';
                    redirect('Candidate/olevel2', 'refresh');
//                    $this->template->load('Candidate', 'default', 'olevel1', $this->data);
    ...
    function olevel2()
    {
        ...
        {//when there is input from user
            ...
            if($this->form_validation->run() == false)
            {//check validation failure first for security
				//echo "<script lang='javascript' type='text/javascript' >alert('Olevel 2nd Sitting Empty. Guess You Dont Have It'); </script>";
				//for based on school
				...
				if($school === 'midwifery')
				{
					...
					redirect('Candidate/preview', 'refresh');
				}
                
            }elseif($this->form_validation->run() == true){//validation successful
