...
use Medlab_candidate;
use Illuminate\Support\Facades\Storage;
use App\Http\Controllers\Controller;
use App\Medlab_candidate_pin;
//use Illuminate\Contracts\Validation\Validator;
use Auth;
use Illuminate\Support\Facades\DB;
use App\Medlab_candidates_profiles;
use Image;
...
class Medlab_candidateController extends Controller
{
  /**
    ...
    {
      //specify guard
        $this->middleware('auth:medlab_candidate');
    }
    ...
    {
        return view('backend.medlab.candidate.index');
    }
    ...
    */
    public function showProfile_updateForm()
    {
      ...
      return view('backend.medlab.candidate.profile',['countries' => $countries,'states' => $states,]);
      //return redirect()->route('medlab_candidate_profile_update')->with('countries', $countries);
    }
    ...
    */
    public function profile_update(Request $request)
    {
          ...
          $profile = DB::table('medlab_candidates_profiles')->select("dob", 'gender', 'origin_state', 'birth_town')->where('user_id', Auth::user()->id)->get();
          ...
          if(!empty($profile))
          {//update profile
                DB::table('medlab_candidates_profiles')
            ...
            //insert fresh data into candidates_profile
              DB::table('medlab_candidates_profiles')->insert(
                  [
      ...
      return redirect(route('medlab_candidate_passport_upload')); //'submited'; //view('backend.medlab.candidate.profile');
    }
  ...
  /*public function messages()
  {
    ...
    */
    public function passport_upload(Request $request)
    {
        ...
        if (Medlab_candidates_profiles::select('passport')
                                  ->where('passport', '<>', '')
                                  ->where('passport', '<>', NULL)
                                  ->where('user_id', '=', Auth::user()->id)
                                  ->exists()) {
          ...
          $passport = Medlab_candidates_profiles::select('passport')
                                    ->where('passport', '<>', '')
                                    ->where('user_id', '=', Auth::user()->id)
                                    ->get();
          ...
          chdir('images/candidates/medlab/');
          ...
          $file->move('images/candidates/medlab', $passport); // move it to the 'uploads' directory (public/uploads)
          ...
          $img = Image::make('images/candidates/medlab/'.$passport);
          ...
          return redirect(route('medlab_candidate_contact')); //true; //return 'done';
          ...
          //insert passport column in medlab_candidates_profiles
          ...
          // create instance of Intervention Image
          $img = Image::make('public/candidates/medlab/'.$passport);
    ...
    */
    ...
    {
      return view('backend.medlab.candidate.passport_upload');
    }
    ...
    */
    public function contact(Request $request)
    {
      ...
      //fetch already existing database data
      $contacts = DB::table('medlab_candidates_contacts')
      ...
      {//update profile
        ...
        ->update(['votes' => 1]);
        */
        DB::table('medlab_candidates_contacts')
            ->where('user_id', Auth::user()->id)
            ->update(
            [
        ...
        return redirect(route('medlab_candidate_nextkin'));

      }else{

        //insert fresh data into candidates_profile
          DB::table('medlab_candidates_contacts')->insert(
              [
                'user_id' =>  Auth::user()->id,
                'contact_address' => $request['contact_add'],
                'permanent_address' => $request['permanent_add'],
    ...
    */
    ...
    {
      return view('backend.medlab.candidate.contact');
    }
    ...
    */
    ...
    {
      return view('backend.medlab.candidate.nextkin');
    }
    ...
    */
    public function nextkin(Request $request)
    {
        ...
        //fetch already existing database data
        $nextkin = DB::table('medlab_candidates_nextkins')
                    ->select('relationship')
                    ->where('user_id', Auth::user()->id)->get();
        ...
        {//update profile
          ...
          ->update(['votes' => 1]);
          */
          DB::table('medlab_candidates_nextkins')
              ->where('user_id', Auth::user()->id)
              ->update(
              [
                'user_id' =>  Auth::user()->id,
          ...
          return redirect(route('medlab_candidate_olevel1'));

        }else{

          //insert fresh data into candidates_profile
            DB::table('medlab_candidates_nextkins')->insert(
                [
                  'user_id' =>  Auth::user()->id,
                  'name' => $request['next_kin_name'],
                  'nextkin_address' => $request['next_kin_address'],
    ...
    */
    ...
    {
      return view('backend.medlab.candidate.password_change');
    }
    ...
    */
    public function password_change(Request $request)
    {
      ...
      return 'submited'; //view('backend.medlab.candidate.profile');
    }
    ...
    */
    ...
    {
      return view('backend.medlab.candidate.olevel1');
    }
    ...
    */
    public function olevel1(Request $request)
    {
      ...
      //fetch already existing database data
      $olevel1 = DB::table('medlab_candidates_olevels1')
                  ->select('*')
                  ->where('user_id', Auth::user()->id)->get();
      ...
      if(!empty($olevel1))
      {
        //update already existing olevel1 data
          DB::table('medlab_candidates_olevels1')
              ->where('user_id', Auth::user()->id)
              ->update(
              [
                'user_id' =>  Auth::user()->id,
        ...
        return redirect(route('medlab_candidate_olevel2'));

      }else{

        //insert fresh data into candidates_profile
          DB::table('medlab_candidates_olevels1')->insert(
              [
                'user_id' =>  Auth::user()->id,
                'examination' => $request['examination'],
                'exam_number' => $request['exam_number'],
    ...
    */
    public function olevel2(Request $request)
    {
      ...
      //fetch already existing database data
      $olevel2 = DB::table('medlab_candidates_olevels2')
                  ->select('*')
                  ->where('user_id', Auth::user()->id)->get();
      ...
      {
        //update already existing olevel1 data
          DB::table('medlab_candidates_olevels2')
              ->where('user_id', Auth::user()->id)
              ->update(
              [
                'user_id' =>  Auth::user()->id,
        ...
        return redirect(route('medlab_candidate_preview'));
        //return redirect(route('medlab_candidate_olevel2'));

      }else{

        //insert fresh data into candidates_profile
          DB::table('medlab_candidates_olevels2')->insert(
              [
                'user_id' =>  Auth::user()->id,
                'examination' => $request['examination'],
                'exam_number' => $request['exam_number'],
    ...
    */
    ...
    {
      return view('backend.medlab.candidate.olevel2');
    }
    ...
    public function get_state($state_code)
    {
        ...
        {
            //fetch user profile
              $profile = DB::table('medlab_candidates_profiles')->select("*")->where('user_id', Auth::user()->id)->get();
              $profile = json_decode(json_encode($profile));
              $profile = json_decode(json_encode($profile), true);
        ...
        {
          //next of kin details
          $nextkins = DB::table('medlab_candidates_nextkins')->select('*')->where('user_id', Auth::user()->id)->get();
          ...
          {
                //fetch user olevel1
                $olevel1 = DB::table('medlab_candidates_olevels1')->select("*")->where('user_id', Auth::user()->id)->get();
                $olevel1 = json_decode(json_encode($olevel1));
                $olevel1 = json_decode(json_encode($olevel1), true);
        ...
        {
              //fetch user olevel2
              $olevel2 = DB::table('medlab_candidates_olevels2')->select("*")->where('user_id', Auth::user()->id)->get();
              $olevel2 = json_decode(json_encode($olevel2));
              $olevel2 = json_decode(json_encode($olevel2), true);
        ...
        {
          $contacts = DB::table('medlab_candidates_contacts')->select('*')->where('user_id', Auth::user()->id)->get();
          $contacts = json_decode(json_encode($contacts));
          $contacts = json_decode(json_encode($contacts), true);
    ...
    */
    public function showPreviewForm()
    {
      ...
      return view('backend.medlab.candidate.preview', [
    ...
    */
    public function showComplete_applicationForm()
    {
      ...
      'contacts' => $contacts,
      'nextkin' => $nextkin,*/
      return view('backend.medlab.candidate.complete_application',
                  [
    ...
    public function get_passport($id)
    {
        ...
        $passport = DB::table('medlab_candidates_profiles')->select('passport')->where('user_id', Auth::user()->id)->get();
        $passport = json_decode(json_encode($passport));
        $passport = json_decode(json_encode($passport), true);
